...
using RiotRtmpDto.Platform.Reroll.Pojo;
using RiotRtmpDto.Platform.Catalog.Icon;
using RiotRtmpDto.Kudos.Dto;
using Newtonsoft.Json;
using RiotRtmpDto.Platform.Summoner.Spellbook;
using RiotRtmpDto.Platform.Trade.Api.Contract;
using RiotRtmpDto.Platform.Summoner.Icon;
using RiotRtmpDto.Platform.Summoner.Masterybook;
using RiotRtmpDto.Platform.Matchmaking;
using RiotRtmpDto.Platform.Game.Practice;
using RiotRtmpDto.Platform.Statistics;
using RiotRtmpDto.Team;
using RiotRtmpDto.Platform.Statistics.Team;
using RiotRtmpDto.Platform.Summoner;
using RiotRtmpDto.Team.Dto;
using RiotRtmpDto.Platform.Leagues.Client.Dto;
using RiotRtmpDto.Leagues.Pojo;
using RiotRtmpDto.Platform.Summoner.Runes;
using RiotRtmpDto.Platform.Catalog.Champion;
using RiotRtmpDto.Platform.Summoner.Boost;
using RiotRtmpDto.Platform.Harassment;
using RiotRtmpDto.Platform.Login;
using RiotRtmpDto.Platform.Clientfacade.Domain;
using LegacyPVP.Logic;

namespace LegacyPVP.RtmpsLogic
{
    public class RiotCalls
    {
        ...
        public static string GetAuthKey(String Username, String Password, String LoginQueue)
        {
            ...
            Dictionary<string, object> Json = JsonConvert.DeserializeObject<Dictionary<string, object>>(sb.ToString());
        ...
        public static string GetIpAddress()
        {
            ...
            Dictionary<string, string> IpAddress = JsonConvert.DeserializeObject<Dictionary<string, string>>(sb.ToString());
        ...
        public static SerializationContext RegisterObjects()
        {
            ...
            Assembly LegacyPVP = typeof(PotentialTradersDTO).Assembly;

            var x = LegacyPVP.GetTypes().Where(t => String.Equals(t.Namespace, "RiotRtmpDto.Platform", StringComparison.Ordinal));
            foreach (Type Platform in x)
                context.Register(Platform);

            x = LegacyPVP.GetTypes().Where(t => String.Equals(t.Namespace, "RiotRtmpDto.Leagues", StringComparison.Ordinal));
            foreach (Type League in x)
                context.Register(League);

            x = LegacyPVP.GetTypes().Where(t => String.Equals(t.Namespace, "RiotRtmpDto.Team", StringComparison.Ordinal));
